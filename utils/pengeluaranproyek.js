...
const list = ({ id_proyek, monthyear, start, end, id_vendor }) => {
  const sql = `Select i.nama instansi,v.nama vendor, pk.id_produkmasuk, pp.id_produkkeluar, pr.id id_proyek, pr.nama namaproyek, pr.id_instansi, pp.id id_pengeluaranproyek, pp.tanggal tanggalpengeluaran, pp.jumlah, pp.harga hargapengeluaran, pp.status, pp.lunas, pp.keterangan keteranganpp, k.nama namakaryawan, p.*, m.nama merek From ${table} pp left join produk p on pp.id_produk = p.id left join karyawan k on pp.id_karyawan = k.id left join proyek pr on pp.id_proyek=pr.id left join merek m on m.id=p.id_merek left join vendor v on v.id=pp.id_vendor left join produkkeluar pk on pk.id=pp.id_produkkeluar left join instansi i on pr.id_instansi=i.id where 1=1 ${
