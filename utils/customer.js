...
const list = ({ limit }) => {
  ...
  const sql = `select i.id, i.nama, i.alamat, coalesce((pm.pembayaran-p.produksi),0) provit, coalesce(jumlah_proyek,0) jumlah_proyek from ${table} i left join (${pembayaran}) pm on i.id=pm.id left join (${produksi}) p on i.id=p.id order by nama ${
    limit ? "limit ?" : ""
  }`;
  const values = [];
