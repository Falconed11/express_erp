-- 1. Refactor 'laporan' table
ALTER TABLE laporan DROP COLUMN id_parent,
    DROP FOREIGN KEY fk_laporan,
    DROP FOREIGN KEY fk_laporan_coa_filter,
    DROP FOREIGN KEY fk_laporan_coa,
    DROP COLUMN id_coa_filter,
    DROP COLUMN id_coa,
    DROP COLUMN modifier,
    ADD COLUMN isReport TINYINT(1) NOT NULL DEFAULT 0
AFTER nama;
-- 2. Adjust COA tables to auto-increment
ALTER TABLE coa_type
MODIFY COLUMN id INT NOT NULL AUTO_INCREMENT;
ALTER TABLE coa_subtype
MODIFY COLUMN id INT NOT NULL AUTO_INCREMENT;
-- 3. Create the relation table
...
ALTER TABLE laporan_relation
ADD CONSTRAINT unique_laporan_relation_id_parent_id_child UNIQUE (id_parent, id_child);
ALTER TABLE laporan
ADD CONSTRAINT unique_laporan_nama UNIQUE (nama);